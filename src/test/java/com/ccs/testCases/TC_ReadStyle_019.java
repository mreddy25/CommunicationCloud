...
public class TC_ReadStyle_019 extends BaseClass {
	@Test(dataProvider="StylesData")
	public void readStyles(String uname, String pwd, String sName, String styleName, String styleLName, String styleDesc, String attrbType, String AttrbName) throws InterruptedException, IOException
	{
		...
		//logger.info("Style Date is: "+elements.get(4).getText());
/*		for (int i=0; i<elements.size();i++){
			logger.info("Styles details are:" + elements.get(i).getText());
		    }
*/			
		//expand StyleAttibute	
		...
		logger.info("Style Attibute is: "+elements.get(3).getText());
		
		//expand status date
		if (sp.statusDate().isDisplayed())
		{
			sp.statusDate().click();
			logger.info("Clicked on status date to expand");
			Thread.sleep(3000);
		}
		
		//verify style status date
		logger.info("Style Status Date is: "+elements.get(4).getText());
